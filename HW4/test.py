...
def evaluate(network, dataloader, netG, netD, device, dim_noise):
    ...
    for i, (real_image, label) in enumerate(dataloader):
        ...
        real_images_ = torch.nn.functional.interpolate(real_image, size=(299, 299), mode='bilinear', align_corners=False)
        real_images_ = (real_images_ * 255).to(torch.uint8)
        fake_images_ = torch.nn.functional.interpolate(fake_image, size=(299, 299), mode='bilinear', align_corners=False)
