...
class SpokenSQuADDataset(Dataset):
    ...
    def preprocess_train(self, sample):
        context, question, answer, answer_start, qid = sample['context'], sample['question'], sample['answer'], sample['answer_start'], sample['id']
        ...
        for i in range(len(tokenized_context) - len(tokenized_answer) + 1):
            if tokenized_context[i:i+len(tokenized_answer)] == tokenized_answer:
                ...
                break
        if start_pos == -1:
            tokenized_before_answer = self.tokenizer.encode(context[:answer_start], add_special_tokens=False, max_length=512, truncation=True)
            tokenized_by_answer = self.tokenizer.encode(context[:answer_start+len(answer)], add_special_tokens=False, max_length=512, truncation=True)
        ...
        if len(tokenized_context_segment) < self.window_size:
            tokenized_context_segment_ = tokenized_context_segment + [self.tokenizer.pad_token_id] * (self.window_size - len(tokenized_context_segment))
        ...
        if len(tokenized_question) < self.question_length:
            tokenized_question_ = tokenized_question + [self.tokenizer.pad_token_id] * (self.question_length - len(tokenized_question))
